def travel(r, zipcode):
    ...
    streets, house = [], []
    for add in address:
        if zipcode and zipcode in add:
            add = add.split()
            house.append(add[0])
            streets.append(add[1:-2])
    
    return zipcode + ':' + ','.join([' '.join(street) for street in streets]) + '/' + ','.join(house) 

# filter(lambda x: bool(zipcode) and zipcode in x, r.split(","))
